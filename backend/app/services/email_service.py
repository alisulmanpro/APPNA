...
class APPNAEmail:
    """
    Central email handler for APPNA platform.

    Usage:
        email = APPNAEmail()
        await email.send(EmailType.WELCOME, WelcomePayload(
            to="member@appna.org",
            name="Ahmed Khan",
            role="Committee Chair",
            login_link="https://appna.org/login"
        ))
    """

    FROM_EMAIL  = "APPNA <noreply@appna.org>"
    BRAND_COLOR = "#1e40af"
    BRAND_NAME  = "APPNA"
    ...
    async def send(self, email_type: EmailType, payload: EmailPayload) -> dict:
        ...
        try:
            ...
            return {"success": True, "id": response["id"], "type": email_type}
        except Exception as e:
            return {"success": False, "error": str(e), "type": email_type}
    ...
    def _get_handler(self, email_type: EmailType):
        ...
        if not handler:
            raise ValueError(f"No handler for email type: {email_type}")
        return handler
            ...
            <table width="100%" cellpadding="0" cellspacing="0"
                ...
                <tr><td align="center">
                    <table width="600" cellpadding="0" cellspacing="0"
                        ...
                        <!-- Header -->
                        <tr>
                            <td style="background:{self.BRAND_COLOR};padding:32px 40px;
                                       text-align:center;">
                                <h1 style="margin:0;color:#ffffff;font-size:24px;
                                           font-weight:700;">{self.BRAND_NAME}</h1>
                        ...
                        <tr>
                            <td style="background:#f9fafb;padding:24px 40px;
                                ...
                                <p style="margin:4px 0 0;color:#9ca3af;font-size:12px;">
                                    This email was sent to you as a member of APPNA.
                                </p>
                            </td>
                        </tr>
    ...
    # ── Reusable components ────────────────────────────────────
    def _button(self, text: str, url: str, color: str = None) -> str:
        bg = color or self.BRAND_COLOR
        return f"""
        <div style="text-align:center;margin:32px 0;">
            <a href="{url}" style="background:{bg};color:#ffffff;padding:14px 32px;
    ...
    def _badge(self, text: str, color: str) -> str:
        return f"""<span style="background:{color};color:#fff;padding:4px 12px;
                    border-radius:20px;font-size:12px;font-weight:600;">{text}</span>"""
    ...
    def _alert_box(self, message: str,
                   color: str = "#fef3c7", border: str = "#f59e0b") -> str:
        return f"""
    ...
    def _info_table(self, rows: list[tuple], bg: str = "#f9fafb") -> str:
        ...
        return f"""
        <table width="100%" cellpadding="0" cellspacing="0"
               style="background:{bg};border-radius:8px;padding:20px;margin:20px 0;">
            {content}
        </table>
        """
    ...
    def _activation(self, p: ActivationPayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">Activate Your Account</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Welcome to APPNA, <strong>{p.name}</strong>!
    ...
    def _password_reset(self, p: PasswordResetPayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">Reset Your Password</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Hi <strong>{p.name}</strong>, we received a password reset request.
        </p>
    ...
    def _email_verification(self, p: ActivationPayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">Verify Your Email</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Hi <strong>{p.name}</strong>, please verify your email address to continue.
        </p>
        {self._button("Verify Email Address", p.activation_link)}
        """
        return "Verify Your APPNA Email", self._base(content, "Verify your email")

    def _welcome(self, p: WelcomePayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">Welcome to APPNA! 🎉</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Hi <strong>{p.name}</strong>, your account is ready.
            You have been assigned the role of <strong>{p.role}</strong>.
        </p>
        <div style="background:#eff6ff;border-radius:8px;padding:20px;margin:20px 0;">
            ...
            <ul style="color:#374151;font-size:14px;margin:10px 0 0;padding-left:20px;">
                ...
                <li>Meeting scheduling & AI summaries</li>
                <li>Task tracking & accountability</li>
    ...
    def _account_deactivated(self, p: EmailPayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">Account Deactivated</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Hi <strong>{p.name}</strong>, your APPNA account has been deactivated.
    ...
    def _meeting_scheduled(self, p: MeetingPayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">Meeting Scheduled</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 8px;">
            Hi <strong>{p.name}</strong>, a new meeting has been scheduled.
        </p>
        <h3 style="color:{self.BRAND_COLOR};font-size:18px;margin:0 0 16px;">
            {p.meeting_title}
        </h3>
    ...
    def _meeting_updated(self, p: MeetingPayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">Meeting Updated</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 8px;">
            Hi <strong>{p.name}</strong>, a meeting has been updated.
        </p>
    ...
    def _meeting_cancelled(self, p: MeetingPayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">Meeting Cancelled</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 8px;">
            Hi <strong>{p.name}</strong>, the following meeting has been cancelled.
        </p>
        <h3 style="color:#dc2626;font-size:18px;margin:0 0 16px;">
            {p.meeting_title} {self._badge("CANCELLED", "#dc2626")}
        </h3>
        {self._meeting_info(p)}
    ...
    def _meeting_reminder(self, p: MeetingPayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">⏰ Meeting Reminder</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 8px;">
            Hi <strong>{p.name}</strong>, your meeting is coming up soon.
        </p>
        <h3 style="color:{self.BRAND_COLOR};font-size:18px;margin:0 0 16px;">
            {p.meeting_title}
        </h3>
    ...
    def _meeting_summary(self, p: MeetingPayload) -> tuple:
        summary_block = f"""
        <div style="background:#eff6ff;border-radius:8px;padding:20px;margin:20px 0;">
        ...
        """ if p.ai_summary else ""

        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">Meeting Summary</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 8px;">
            Hi <strong>{p.name}</strong>, here is the summary for the completed meeting.
        </p>
        <h3 style="color:{self.BRAND_COLOR};font-size:18px;margin:0 0 16px;">
            {p.meeting_title} {self._badge("COMPLETED", "#059669")}
        </h3>
    ...
    def _task_info(self, p: TaskPayload) -> str:
        ...
        priority_html = f'<span style="color:{color};font-weight:600;">{p.priority.upper()}</span>'
        return self._info_table([
            ("📋 Task", p.task_title),
            ("🏛 Committee", p.committee_name),
            ("📅 Due Date", p.due_date),
    ...
    def _task_assigned(self, p: TaskPayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">New Task Assigned</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Hi <strong>{p.name}</strong>, a new task has been assigned to you.
        </p>
        {self._task_info(p)}
        <p style="color:#374151;font-size:14px;margin:16px 0;">
            <strong>Description:</strong> {p.task_description}
        </p>
        {self._button("View Task", p.task_link)}
        """
        return f"New Task: {p.task_title}", self._base(content, "You have a new task")

    def _task_due_soon(self, p: TaskPayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">Task Due Soon ⏰</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Hi <strong>{p.name}</strong>, a task is due soon.
        </p>
    ...
    def _task_overdue(self, p: TaskPayload) -> tuple:
        content = f"""
        <h2 style="color:#dc2626;font-size:22px;margin:0 0 8px;">Task Overdue ❗</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Hi <strong>{p.name}</strong>, the following task is overdue.
        </p>
    ...
    def _task_completed(self, p: TaskPayload) -> tuple:
        content = f"""
        <h2 style="color:#059669;font-size:22px;margin:0 0 8px;">Task Completed ✅</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Hi <strong>{p.name}</strong>, a task has been marked as completed.
        </p>
        {self._task_info(p)}
        """
    ...
    def _document_submitted(self, p: DocumentPayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">
            Document Submitted for Approval
        </h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Hi <strong>{p.name}</strong>, a document needs your approval.
        </p>
        {self._info_table([
    ...
    def _document_approved(self, p: DocumentPayload) -> tuple:
        content = f"""
        <h2 style="color:#059669;font-size:22px;margin:0 0 8px;">Document Approved ✅</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Hi <strong>{p.name}</strong>, your document has been approved.
        </p>
        {self._info_table([
            ("📄 Document", p.document_title),
            ("🏛 Committee", p.committee_name),
    ...
    def _document_rejected(self, p: DocumentPayload) -> tuple:
        content = f"""
        <h2 style="color:#dc2626;font-size:22px;margin:0 0 8px;">Document Rejected</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Hi <strong>{p.name}</strong>, your document has been rejected.
        </p>
        {self._info_table([
            ("📄 Document", p.document_title),
            ("🏛 Committee", p.committee_name),
    ...
    def _committee_added(self, p: CommitteePayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">Added to Committee</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Hi <strong>{p.name}</strong>, you have been added to a committee.
        </p>
        {self._info_table([
            ("🏛 Committee", p.committee_name),
            ("👤 Your Role", p.role),
            ("✅ Added By", p.assigned_by),
        ], "#eff6ff")}
        """
        return f"Added to {p.committee_name}", self._base(content, "Added to committee")

    def _committee_removed(self, p: CommitteePayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">Removed from Committee</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Hi <strong>{p.name}</strong>, you have been removed from a committee.
        </p>
        {self._info_table([
            ("🏛 Committee", p.committee_name),
            ("✅ Removed By", p.assigned_by),
        ])}
        """
        return f"Removed from {p.committee_name}", self._base(content, "Committee membership update")

    def _chair_assigned(self, p: CommitteePayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">
            You are now Committee Chair 🎉
        </h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Congratulations <strong>{p.name}</strong>!
            You have been assigned as the Chair of <strong>{p.committee_name}</strong>.
        </p>
        {self._info_table([
            ("🏛 Committee", p.committee_name),
            ("👑 Role", "Committee Chair"),
    ...
    def _broadcast(self, p: BroadcastPayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">📢 Announcement</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Hi <strong>{p.name}</strong>, you have a new announcement from APPNA leadership.
        </p>
    ...
    def _event_announcement(self, p: BroadcastPayload) -> tuple:
        content = f"""
        <h2 style="color:#111827;font-size:22px;margin:0 0 8px;">🎉 Event Announcement</h2>
        <p style="color:#6b7280;font-size:15px;margin:0 0 24px;">
            Hi <strong>{p.name}</strong>, APPNA has a new event for you.
        </p>
        <div style="background:#eff6ff;border-radius:8px;padding:24px;margin:20px 0;">
            <h3 style="margin:0 0 12px;color:{self.BRAND_COLOR};font-size:18px;">
                {p.subject}
            </h3>
            <p style="margin:0;color:#374151;font-size:14px;line-height:1.7;">{p.message}</p>
        </div>
